...
from markuplift.annotation import TYPE_ANNOTATION_KEY, Annotations, annotate_elements_in_mixed_content_as_inline, BLOCK_TYPE_ANNOTATION, INLINE_TYPE_ANNOTATION
...
def test_mixed_content_with_text():
    ...
    annotate_elements_in_mixed_content_as_inline(tree, annotations)
    for elem in tree.getroot():
        assert annotations.annotation(elem, TYPE_ANNOTATION_KEY) == INLINE_TYPE_ANNOTATION
...
def test_mixed_content_with_some_block_annotated():
    ...
    annotations.annotate(block, TYPE_ANNOTATION_KEY, BLOCK_TYPE_ANNOTATION)
    annotate_elements_in_mixed_content_as_inline(tree, annotations)
...
def test_mixed_content_with_comment_and_pi_annotated_block():
    ...
    annotations.annotate(comment, TYPE_ANNOTATION_KEY, BLOCK_TYPE_ANNOTATION)
    annotations.annotate(pi, TYPE_ANNOTATION_KEY, BLOCK_TYPE_ANNOTATION)
    annotate_elements_in_mixed_content_as_inline(tree, annotations)
    ...
    assert annotations.annotation(block, TYPE_ANNOTATION_KEY) == INLINE_TYPE_ANNOTATION
    assert annotations.annotation(tree.getroot()[3], TYPE_ANNOTATION_KEY) == INLINE_TYPE_ANNOTATION
...
def test_elements_interleaved_with_significant_text():
    ...
    comment = tree.getroot()[1]
    pi = tree.getroot()[2]
    assert annotations.annotation(em, TYPE_ANNOTATION_KEY) == "inline"
    assert annotations.annotation(comment, TYPE_ANNOTATION_KEY) == "inline"
    assert annotations.annotation(pi, TYPE_ANNOTATION_KEY) == "inline"
